...
class OpenAPVizWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        from .constants import MIN_DURATION_MIN, MAX_DURATION_MIN
    ...
    def render(self):
        ...
        if hasattr(self.env, 'nominal_route') and self.env.nominal_route:
            points = [self._to_pixel(wp['lat'], wp['lon']) for wp in self.env.nominal_route]
            if len(points) > 1:
                pygame.draw.lines(self.screen, (70, 70, 100), False, points, 2)
            
            for i, p in enumerate(points):
                # Le waypoint cible actuel est en vert, les autres en gris
                color = (0, 255, 127) if i == self.env.current_waypoint_idx else (150, 150, 150)
